...
import {
  ...
  ObjDoiTuong,
  DuyetUserDoiTuong,
  DuyetUserDoiTuongDto,
  DuyetUserDoiTuongServiceServiceProxy,
  DuyetUserDoiTuongViewDto,
  UserServiceProxy,
  UserViewDto,
...
@Component({
  selector: "app-user-doituong-duyet",
  templateUrl: "./user-doituong-duyet.component.html",
  styleUrls: ["./user-doituong-duyet.component.css"],
})
export class UserDoituongDuyetComponent
  extends AppComponentBase
  implements OnInit
{
  form: FormGroup;
  users: DuyetUserDoiTuongViewDto[] = [];
  keyword = "";
  ...
  doituongUser: DuyetUserDoiTuongDto = new DuyetUserDoiTuongDto();
  saving = false;
  userid: 0;
  styleUser: any;
  objSelected: ObjDoiTuong[] = [];
  userdoituongs: DuyetUserDoiTuong[] = [];
  keyWord: string;
  orgId: number;
  ...
  constructor(
    ...
    private _duyetuserDoiTuongService: DuyetUserDoiTuongServiceServiceProxy,
  ...
  getUser(): void {
    ...
    this._duyetuserDoiTuongService
  ...
  getUserDoiTuong(): void {
    this._duyetuserDoiTuongService.getUserDoiTuong().subscribe((result) => {
      result.forEach((rs) => {
    ...
    this._duyetuserDoiTuongService
      ...
      .subscribe((result) => {
        result.forEach((rs) => {
          ...
          input.name = rs.name;
          input.isActive = true;
          input.isChecked = rs.isChecked;
          this.doituonginfoInput.push(input);
        });
      });
    //console.log(this.doituonginfoInput)
  }
  ...
  save() {
    ...
    // else
    //   if (this.doituongUser.objDoiTuong.length == 0) {
    //     this.notify.error("Chưa chọn đối tượng nào để cấu hình !");
    //     return;
    //   }
    //nên check sâu hơn theo từng đối tượng nhỏ để thông báo
    ...
    if (this.update == true) {
      ...
      if (error == false) {
        this._duyetuserDoiTuongService
          .updateAsyncDuyet(this.doituongUser)
          .subscribe((rs) => {
