...
export class UploadImageComponent extends AppComponentBase implements OnInit {
  progress: number;
  res:any;
  fileInfos:FileInfo[] =[];
 ...
 max_upload_file:string;
  ...
  ngOnInit() {
    this.configService.getValueByKey("MAX_UPLOAD_IMAGE").subscribe(rs=>{
      this.max_upload_file=rs!=null?rs:Const.max_upload_image;
    })
  }
  ...
  onLoadImage(fileLoads: Media[], edit?: boolean): void {
    edit = true;
    //this.initImageList();
